...
def get_is_operator():
    global is_operator
    global is_operator_L
    if bpy.context.scene.leftWindowObj == '右耳':
        return is_operator
    else:
        return is_operator_L
    ...
    for vert in bm.verts:
        ...
        if localLaHouDu_cur:
            ...
            if (thickness > maxHoudu or thickness < minHoudu) and (index in vertex_set):
                ...
                # 根据理论厚度修改坐标
                if length > 0:
                    bm.verts[index].co = ori_bm.verts[index].co - \
...
class Thickening(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if bpy.context.screen.areas[0].spaces.active.context == 'RENDER':
            ...
            elif bpy.context.scene.var != 1 and bpy.context.scene.var in get_process_var_list("打磨"):
                if op_cls.__timer:
...
class Thinning(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if bpy.context.screen.areas[0].spaces.active.context == 'RENDER':
            ...
            elif bpy.context.scene.var != 2 and bpy.context.scene.var in get_process_var_list("打磨"):
                if op_cls.__timer:
                    context.window_manager.event_timer_remove(op_cls.__timer)
                    op_cls.__timer = None
                print("打薄modal结束")
...
class Smooth(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if bpy.context.screen.areas[0].spaces.active.context == 'RENDER':
            ...
            elif bpy.context.scene.var != 3 and bpy.context.scene.var in get_process_var_list("打磨"):
                if op_cls.__timer:
                    context.window_manager.event_timer_remove(op_cls.__timer)
                    op_cls.__timer = None
                print("平滑modal结束")
