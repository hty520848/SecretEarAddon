...
def smooth():
    ...
    for vert in bm.verts:
        ...
        if distance <= b and xmin < vert.co.x < xmax:
...
# 获取截面半径
def getRadius():
    global old_radius, scale_ratio, now_radius, onobj
...
class Circle_Cut(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        global scale_ratio, zmax, zmin, onobj
        op_cls = Circle_Cut
...
class Reset_Cut(bpy.types.Operator):
    ...
    def excute(self, context):
        ...
        if (qiegeenum == 2):
            ...
            bpy.data.objects['StepCutSphere3'].location = (11.282, -2.077, 6.607)
            bpy.data.objects['StepCutSphere4'].location = (8.107, -6.029, 13.345)
            bpy.ops.object.stepcut('INVOKE_DEFAULT')
