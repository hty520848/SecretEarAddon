...
class LocalReset(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        bpy.ops.paint.mask_flood_fill(mode='VALUE',value=1)          #将物体用遮罩填充，由于遮罩区域无法操作，因此非遮罩区域就是选中的局部加厚区域
        return {'FINISHED'}
...
class AddArea(bpy.types.Operator):
    ...
    def execute(self,context):
        if bpy.context.mode == "OBJECT":                                        #将默认的物体模式切换到雕刻模式
                bpy.ops.sculpt.sculptmode_toggle()               
        bpy.ops.wm.tool_set_by_id(name="builtin_brush.Mask")                    #进入遮罩模式，笔刷方向设置为扩大区域
...
class ReduceArea(bpy.types.Operator):
    ...
    def execute(self,context):
        if bpy.context.mode == "OBJECT":                                       #将默认的物体模式切换到雕刻模式
            bpy.ops.sculpt.sculptmode_toggle()               
        bpy.ops.wm.tool_set_by_id(name="built_brush.Mask")                     #进入遮罩模式，笔刷方向设置为缩小区域
