...
from .shell_eardrum.shell_eardrum_bottom_fill import set_shell_modal_finish, copy_model_for_bottom_curve, \
    reset_after_bottom_curve_change, update_shell_after_curve_change, record_state
from .shell_eardrum.shell_canal import createShellCanal, updateArmaturePlaneLocationAndNormal

from .parameters_for_create_mould import set_right_hard_eardrum_border, set_left_hard_eardrum_border, \
    set_right_soft_eardrum_border, set_left_soft_eardrum_border, set_right_frame_style_eardrum_border, \
    set_left_frame_style_eardrum_border, set_right_frame_style_hole_border, set_left_frame_style_hole_border, \
    set_right_shell_plane_border, set_left_shell_plane_border, set_right_shell_border, set_left_shell_border

from ..tool import recover_and_remind_border, moveToRight, moveToLeft, utils_re_color, convert_to_mesh, \
    recover_to_dig, newColor, getOverride, extrude_border_by_vertex_groups, track_time
...
from ..parameter import get_switch_time, set_switch_time, get_switch_flag, set_switch_flag, check_modals_running, \
    ...
    set_smooth_curve_modal_start, get_process_var_list, get_torus_modal_running, set_curve_modal_running, \
    get_mirror_context, set_mirror_context, get_shell_curve_obj_active, set_shell_curve_obj_active
...
class MuJudict:
    ...
    waikedictR = {'右耳meshBottomRingBorderR': waikelist1R, '右耳meshPlaneBorderCurve': waikelist12R}
    waikedictL = {'左耳meshBottomRingBorderR': waikelist1L, '左耳meshPlaneBorderCurve': waikelist12L}
    ...
    def reset_frame_curve_dict(self):
        name = bpy.context.scene.leftWindowObj
        if name == '右耳':
            number = 0
            dic = self.kuangjiadictR
            ...
            for i in range(2, number + 1):
                dic.pop(name + 'meshHoleBorderCurve' + str(i))
        elif name == '左耳':
            dic = self.kuangjiadictL
            number = 0
            for key in dic:
...
def join_curve(curve_name, depth, index_initial, index_finish):
    ...
    if enum == 'OP3' and curve_name == name + "PlaneBorderCurve":   # 设置为电池仓的子集
        battery_obj = bpy.data.objects.get(name + "shellBattery")
        child_of_constraint = new_curve_obj.constraints.new(type='CHILD_OF')
...
def update_cut(curve_name, mesh_name, depth):
    ...
    if (enum_name == '软耳模'):
        ...
        recut_success = True
        try:
            soft_generate_cutplane(0.5, 0.5)  # 生成切割平面
            track_time("生成切割平面")
            # get_cut_plane()
            # soft_generate_cutplane(2, 1)  # 生成切割平面
            plane_boolean_cut()
            track_time("布尔切割")
            delete_useless_part()
            track_time("删除多余部分")
        ...
        if (re.match(name + 'HoleBorderCurve', curve_name) != None):  # 上部曲线改变
            resample_number = len(bpy.data.objects[curve_name].data.splines[0].points)
            resample_curve(resample_number, curve_name)
            convert_to_mesh(curve_name, mesh_name, depth)  # 重新生成网格
            number = 0
            ...
            recut_success = True
            try:
                frame_generate_cutplane(0.5, 0.5)  # 生成切割平面
                track_time("生成切割平面")
                # get_cut_plane()
                # frame_generate_cutplane(2, 1)  # 生成切割平面
                plane_boolean_cut()
                track_time("布尔切割")
                delete_useless_part()
                track_time("删除多余部分")
        ...
        recut_success = True
        try:
            hard_generate_cutplane(0.5, 0.5)  # 生成切割平面
            track_time("生成切割平面")
            # get_cut_plane()
            # hard_generate_cutplane(2, 1)  # 生成切割平面
            plane_boolean_cut()
            track_time("布尔切割")
            delete_useless_part()
            track_time("删除多余部分")
        ...
        if curve_name == name + "PlaneBorderCurve":  # 底部曲线改变
            ...
            recut_success = True
            try:
                shell_generate_cutplane(0.5, 0.5)  # 生成切割平面
                track_time("生成切割平面")
                # get_cut_plane()
                # track_time("生成平面")
                plane_boolean_cut()
...
def soft_generate_cutplane(step_number_out, step_number_in):
    ...
    for vert in bm.verts:
        ...
        soft_border.append((vertex_co[0:3], normal[0:3]))
        key = (vertex_co[0], vertex_co[1], vertex_co[2])
        extrude_direction[key] = normal

    if name == '右耳':
        set_right_soft_eardrum_border(soft_border)
    elif name == '左耳':
        set_left_soft_eardrum_border(soft_border)
...
def frame_generate_cutplane(step_number_out, step_number_in):
    ...
    for vert in bm.verts:
        ...
        vertex_co = duplicate_obj.matrix_world @ vert.co
        _, _, normal, _ = target_object.closest_point_on_mesh(vertex_co)
        frame_border.append((vertex_co[0:3], normal[0:3]))
        key = (vertex_co[0], vertex_co[1], vertex_co[2])
        extrude_direction[key] = normal

    if name == '右耳':
        set_right_frame_style_eardrum_border(frame_border)
    elif name == '左耳':
        set_left_frame_style_eardrum_border(frame_border)
...
def hard_generate_cutplane(step_number_out, step_number_in):
    ...
    for vert in bm.verts:
        ...
        vertex_co = duplicate_obj.matrix_world @ vert.co
        _, _, normal, _ = target_object.closest_point_on_mesh(vertex_co)
        hard_border.append((vertex_co[0:3], normal[0:3]))
        key = (vertex_co[0], vertex_co[1], vertex_co[2])
        extrude_direction[key] = normal

    if name == '右耳':
        set_right_hard_eardrum_border(hard_border)
    elif name == '左耳':
        set_left_hard_eardrum_border(hard_border)
...
def shell_generate_cutplane(step_number_out, step_number_in):
    ...
    for vert in bm.verts:
        ...
        vertex_co = duplicate_obj.matrix_world @ vert.co
        _, _, normal, _ = target_object.closest_point_on_mesh(vertex_co)
        shell_border.append((vertex_co[0:3], normal[0:3]))
        key = (vertex_co[0], vertex_co[1], vertex_co[2])
        extrude_direction[key] = normal

    if name == '右耳':
        set_right_shell_border(shell_border)
    elif name == '左耳':
        set_left_shell_border(shell_border)
...
def smoothcurve(curve_name, index, number):
    ...
    for _ in range(1):
        ...
        for i in range(-number, number, 1):
            before_index = (index + i - 1 + point_number) % point_number
            after_index = (index + i + 1 + point_number) % point_number
            smooth_co = (Vector(curve_data.splines[0].points[before_index].co[0:3]) +
                         Vector(curve_data.splines[0].points[after_index].co[0:3])) / 2
            smooth_coords.append((smooth_co[0], smooth_co[1], smooth_co[2], 1))

        for i in range(-number, number, 1):
            smooth_index = (index + i + point_number) % point_number
            curve_data.splines[0].points[smooth_index].co = smooth_coords[i + number]

    # for _ in range(3):
    #     # 预先保存原始坐标（避免迭代干扰）
    #     original_coords = []
    #     for i in range(point_number):
    #         original_coords.append(Vector(curve_data.splines[0].points[i].co[0:3]))
        
    #     # 平滑处理区域
    #     for i in range(-number, number):
    #         current_idx = (index + i + point_number) % point_number
    #         prev_idx = (current_idx - 1 + point_number) % point_number
    #         next_idx = (current_idx + 1) % point_number
            
    #         # 拉普拉斯平滑公式：新位置 = 原始位置*(1-α) + 相邻平均*α
    #         neighbor_avg = (original_coords[prev_idx] + original_coords[next_idx]) / 2
    #         new_co = original_coords[current_idx] * (1 - 0.3) + neighbor_avg * 0.3
            
    #         # 应用新坐标（保留w分量）
    #         curve_data.splines[0].points[current_idx].co = (new_co[0], new_co[1], new_co[2], 1)
...
def changearea(curve_name, index, number):
    name = bpy.context.scene.leftWindowObj
    ...
    bpy.data.objects[name + 'dragcurve'].data.bevel_depth = 0.2


def selectcurve(curve_name, index, number):
    ''' 选择拖拽曲线对象 '''
    name = bpy.context.scene.leftWindowObj
    select_name = name + 'selectcurve'
    if bpy.data.objects.get(select_name) != None:
...
def colorcurve(curve_name, index, number):
    ...
    bpy.data.objects[name + 'coloredcurve'].data.bevel_depth = 0.2
...
def start_curve(color_mode=0):
    ...
    if color_mode == 0:
        ...
        newColor('yellow', 1, 1, 0, 0, 1)
        obj.data.materials.append(bpy.data.materials['yellow'])


def hide_torus():
    set_curve_modal_running(True)
    name = bpy.context.scene.leftWindowObj
...
def reveal_torus():
    ...
    if bpy.context.scene.muJuTypeEnum == 'OP1' or bpy.context.scene.muJuTypeEnum == 'OP4':
        ...
        if bpy.data.objects.get(name + "meshshelloutercanal") is not None:
            createShellCanal()
...
def update_mould_fill():
    ...
    try:
        if mould_type == "OP1":
            ...
            print("常规外壳底部桥接")
            update_shell_after_curve_change()
    ...
    if fill_success and mould_type != "OP2":
        record_state()
...
class TEST_OT_qiehuan(bpy.types.Operator):
    ...
    def invoke(self, context, event):  # 初始化
        ...
        if bpy.data.objects.get(name + 'dragcurve') != None:
            bpy.data.objects.remove(bpy.data.objects[name + 'dragcurve'], do_unlink=True)
        if bpy.data.objects.get(name + 'colorcurve') != None:
            bpy.data.objects.remove(bpy.data.objects[name + 'colorcurve'], do_unlink=True)
    ...
    def modal(self, context, event):
        ...
        if get_torus_modal_running():
            return {'PASS_THROUGH'}

        if bpy.context.scene.var == 19:
            if event.type == 'TWO':
                if event.value == 'RELEASE':
                    if bpy.data.objects.get(name + "point") is None:
            ...
            if (bpy.context.scene.muJuTypeEnum == "OP3" and bpy.data.objects.get(name + "point") is None and
                    get_shell_modal_start()):
                return {'PASS_THROUGH'}
...
class TEST_OT_dragcurve(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        enum = bpy.context.scene.muJuTypeEnum
        #硬耳膜,软耳膜,框架式
        if(enum == 'OP1' or enum == 'OP2' or enum == 'OP4'):
            shell_curve_active = True
        #三种外壳
        else:
            shell_curve_active = get_shell_curve_obj_active()
        if get_mirror_context():
            print('drag finish')
        ...
        if bpy.context.scene.var == 20:
            ...
            if co_on_object(mujudict, context, event) == -1 or co_on_object(mujudict, context, event) == 0:  # 鼠标不在曲线上时
                if (op_cls.__right_mouse_down == False and op_cls.__left_mouse_down == False
                ...
                if event.value == 'RELEASE':
                    if op_cls.__is_moving_right == True:  # 鼠标右键松开
                        ...
                        set_curve_modal_running(False)
                    elif op_cls.__is_moving == True:  # 鼠标左键松开
                        op_cls.__is_moving = False
                        op_cls.__left_mouse_down = False
                        # op_cls.__initial_mouse_x = None
                        # op_cls.__initial_mouse_y = None
                ...
                if event.type == 'MOUSEMOVE':
                    if op_cls.__right_mouse_down == True:  # 鼠标右键按下
                        ...
                        if dis >= 1:
                            ...
                            changearea(op_cls.__curve_name, op_cls.__index, op_cls.__number)
                    ...
                    elif op_cls.__left_mouse_down == True:  # 鼠标左键按下
                ...
                if shell_curve_active:
                    ...
                    if event.type == 'LEFTMOUSE':
                        ...
                        if op_cls.__left_mouse_down == False and op_cls.__right_mouse_down == False:
                            # 鼠标移动时选择不同的曲线区域
                            if op_cls.__is_modifier == False and op_cls.__is_modifier_right == False and op_cls.__curve_name != '' :
                                op_cls.__index = select_nearest_point(op_cls.__curve_name, co)
                                selectcurve(op_cls.__curve_name, op_cls.__index, op_cls.__number)
                                # op_cls.__initial_co = get_co(op_cls.__curve_name, op_cls.__index)
                            elif op_cls.__is_modifier_right == True:
                                op_cls.__is_modifier_right = False

                        elif op_cls.__right_mouse_down == True:  # 鼠标右键按下
                            min_number = max(int(op_cls.__curve_length * 0.01), 3)
                            max_number = int(op_cls.__curve_length * 0.4)
                            op_cls.__is_modifier_right = True
                            # 记录下第一次点击鼠标的位置
            ...
            if co_on_object(mujudict, context, event) == -1 or co_on_object(mujudict, context, event) == 0:  # 鼠标不在曲线上时
                ...
                if shell_curve_active:
                    ...
                    if event.type == 'LEFTMOUSE':
                        ...
                        elif op_cls.__left_mouse_down == True:
                            op_cls.__is_modifier = True
                            index = select_nearest_point(op_cls.__curve_name, co)
                            if abs(index - op_cls.__index) > max(int(op_cls.__number * 0.2), 2):
